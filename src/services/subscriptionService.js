...
export class SubscriptionService {
  ...
  async updateSubscriptionStatuses(userId) {
    const provisioning = await this.sb(`subscriptions?user_id=eq.${encodeURIComponent(userId)}&status=eq.provisioning&select=*`);
    ...
    const provisioned = await this.sb(`subscriptions?user_id=eq.${encodeURIComponent(userId)}&status=eq.provisioned&select=*`);
